...
class TriviaCreator:
    ...
    def check_dm(self, message: Message) -> bool:
        ...
        return message.author == self.current_author and isinstance(
    ...
    async def handle_create_trivia(self, message: Message) -> None:
        ...
        try:
            ...
            response = await self.client.wait_for(
                "message", timeout=60.0, check=self.check_dm
            )
            ...
            # Get URL (optional)
            await message.author.send(
                "Would you like to add a URL for this trivia? (yes/no)"
            )
            url = None
            while True:
                response = await self.client.wait_for(
            ...
            response = await self.client.wait_for(
                "message", timeout=60.0, check=check_difficulty
            )
            ...
            await message.author.send(
                "Now let's add the questions. You must add at least 3 questions."
            )
            await message.author.send(
                "For each question, you'll need to add at least 2 answers."
            )
            ...
            while len(questions) < 3 or (len(questions) < 10):
                ...
                await message.author.send(
                    "\nNow add the answers. You need at least 2 answers and one must be"
                    " correct."
                )
                ...
                while len(answers) < 2 or (len(answers) < 4):
                    ...
                    # Only ask if it is the correct answer if there is not already
                    # a correct answer
                    is_correct = False
                    if not has_correct_answer:
                        await message.author.send(
                            "Is this the correct answer? (yes/no)"
                        )
                        while True:
                            response = await self.client.wait_for(
                                "message", timeout=60.0, check=self.check_dm
                    ...
                    if len(answers) >= 2:
                        # If we already have 2 answers and one is correct, we can
                        # continue
                        if has_correct_answer:
                            await message.author.send(
                                "Do you want to add another answer? (yes/no)"
                            )
                            while True:
                                response = await self.client.wait_for(
                                    "message",
                        ...
                        # Si no hay respuesta correcta aún, forzamos a continuar
                            ...
                            await message.author.send(
                                "You need to mark one answer as correct. "
                                "Let's continue with the next answer."
                            )
                        else:
                            await message.author.send(
                                "You need to mark one answer as correct before "
                                "continuing."
                            )
                            # Removemos la última respuesta para intentar de nuevo
                            answers.pop()

                questions.append({"question_title": question_title, "answers": answers})
            ...
            async with self.trivia_game.api_client:
                try:
                    ...
                    if not response_data.get("id"):
                        ...
                        if "title" in response_data and "already exists" in str(
                            response_data.get("title", [])
                        ):
                            # If the error is due to a duplicate title,
                            # attempt to get a new title
                    ...
                    command_logger.error(
                        f"Data sent: {json.dumps(trivia_data, indent=2)}"
                    )
            ...
            await message.author.send(
                "Time's up for creating the trivia. Try again with $create_trivia"
            )
        except Exception as e:
            command_logger.error(f"Error creating trivia: {e}")
            await message.author.send(
                "An error occurred while creating the trivia. Please try again."
            )
    ...
    async def get_valid_title(self, author) -> str:
        ...
        async with self.trivia_game.api_client:
            try:
                ...
                if existing_trivias:
                    ...
                    existing_titles = {
                        trivia["title"].lower() for trivia in existing_trivias
                    }
                    ...
                    await author.send("\n".join(titles_list))
                    await author.send(
                        "\n⚠️ Please choose a different title from the ones listed "
                        "above."
                    )
            except Exception as e:
                command_logger.error(f"Error fetching existing trivias: {e}")
                await author.send(
                    "⚠️ Could not fetch existing titles, "
                    "but you can continue creating your trivia."
                )
                pass
        ...
        while True:
            ...
            response = await self.client.wait_for(
                "message", timeout=60.0, check=check_dm
            )
