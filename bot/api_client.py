...
class TriviaAPIClient:
    ...
    async def update_score(self, name: str, points: int, discord_channel: str):
        ...
        try:
            ...
            response = await self.post(SCORES_URL, data)
    ...
    async def get_difficulties(self) -> Dict[int, str]:
        """Gets available difficulty options"""
        try:
            url = DIFFICULTY_URL
            return await self.get(url)
        except Exception as e:
            bot_logger.error(f"Error getting difficulties: {e}")
            raise

    async def get_all_trivias(self) -> List[Dict[str, Any]]:
        """Gets all available trivias"""
        try:
            url = TRIVIA_URL
            return await self.get(url)
        except Exception as e:
            bot_logger.error(f"Error getting all trivias: {e}")
            raise